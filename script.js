...
function updateHistoryUI(){
  ...
  el.innerHTML = treeState.history.slice(0,20).map(item=>`
    <div class="history-item" onclick="applyHistoryColor('${item.hex}')">
      ...
      <div class="history-info">
        ...
        <div class="history-time">${new Date(item.timestamp).toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'})} - ${item.rule}</div>
      </div>
    </div>`).join("");
}
...
function clearHistory(){
  ...
  renderTree(); updateSelectedNodeInfo(); updateStats(); updateHistoryUI(); saveToLocalStorage();
...
function saveToLocalStorage(){ try{ localStorage.setItem("conefla-state", JSON.stringify(treeState)); }catch(e){} }
...
function initialize(){
  ...
  picker.addEventListener('change', e=>{ input.value = e.target.value.toUpperCase(); });
  ...
  if(!Object.keys(treeState.nodes).length){
    ...
    renderTree(); updateSelectedNodeInfo();
  }
